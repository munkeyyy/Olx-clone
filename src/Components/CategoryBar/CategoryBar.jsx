...
const CategoryBar = () => {
  ...
  return (
    <div className=" relative flex gap-8 justify-start py-1 lg:px-8 xl:px-32 shadow">
      ...
      <div
        className={`bg-white z-[11] py-10 px-4 absolute left-[20%] shadow top-[20%] translate-x-[-12%] translate-y-[4%] ${
          isRotating ? "block" : "hidden"
        }`}
      >
        <SubCatData isRotating={isRotating} setIsRotating={setIsRotating} />
      </div>
      ...
      <ul className="flex items-center gap-10 list-none">
        {categories.map((category, i) => (
          <li
            ...
            className="text-[.9vw] roboto cursor-pointer  text-black font-normal transition-[all.8s] hover:text-[#00a49f]"
            key={i}
          >
            {category.title}
          </li>
